...
| `suspicious-monotonicity` | 強いハンドより弱いハンドの方が多くプレイされている（下記） |

### `suspicious-monotonicity`（転記ミス検出）

`entry(hand) = 1 - fold` を使い、支配関係にある隣接ハンドの組で
`entry(弱い) - entry(強い) > 0.05` なら警告する。支配関係は **3 種類だけ**：

1. **suited-vs-offsuit** — 同じ 2 ランクならスーテッドが強い（`A7s` > `A7o`）
2. **gap-ladder** — ギャップ 1 / 2 / 3 の各はしごで隣の段（`87s` > `76s`）
3. **pair** — 高いペアが強い（`77` > `66`）

### 採用しなかった 2 規則（重要）

以下は「一見正しいが成立しない」もので、実データで偽陽性を量産した。**再追加しないこと。**

- **highcard**（同じ低ランクで上のカードが高い方が強い、`96s` > `86s`）
  上のカードを上げると**カード間のギャップも広がる**。`86s` はワンギャップ、`96s` はツーギャップで、
  ストレートの作りやすさは `86s` が上。ソルバーが `86s` を開いて `96s` を降りるのは正常な出力。
- **kicker**（同じ高ランクで隣のキッカーが高い方が強い、`A6o` > `A5o`）
  ホイールランク（5 4 3 2）は上のランクが作れないストレート（A2345）を作る。
  `A5o` > `A6o`、`K5s` > `K6s` は実在の挙動。

いずれも「両方のカードが同時に強くなっている」わけではないので、支配関係ではない。
**採用した 3 規則は、強い方が同じ形でカードが両方とも高いか、同一ハンドのスーテッド版**という点で共通する。

**決定：レイズ頻度ではなく entry 頻度（`1 - fold`）で比較する。** SB のようにリンプ分岐を持つ
チャートでは、強いハンドがレイズからリンプへ配分を移すのは正常な戦略なので、レイズ頻度で
比較すると偽陽性が数十件出る（実データで 7 件 → 77 件に増えることを確認した）。

**決定：error ではなく warn。** ここで挙がるのは「ほぼ確実に転記ミス」であって「必ず誤り」ではない。
`A5o` のようにホイールブロッカーで実際に隣接ハンドを上回る例は存在する。判断は人間に残す。

この警告は `CLAUDE.md` §2.1 の方針そのもの — 欠けた値を埋めるのではなく、おかしい入力を指摘する。
